...
const users: { phone: string; code: string | undefined; attempts: number; lastSent: number }[] = [];
...
export const sendCode = (req: Request, res: Response) => {
  ...
  if (!checkAttempts(phone)) {
    return res.status(429).json({ success: false, error: "Превышен лимит попыток. Попробуйте позже." });
  }

  // Проверяем время между отправками
  if (!canResend(phone)) {
    return res.status(429).json({ success: false, error: "Повторная отправка доступна через 30 секунд" });
  }
...
export const verifyCode = (req: Request, res: Response) => {
  ...
  if (!phone || !code) {
    return res.status(400).json({ success: false, error: "Номер телефона и код обязательны" });
  }
  ...
  if (user.code !== code) {
    if (user.attempts >= 3) {
      ...
      return res.status(400).json({ success: false, error: "Превышен лимит попыток. Запросите новый код." });
    }
    return res.status(400).json({ success: false, error: "Неверный код" });
  }
